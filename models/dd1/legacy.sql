...
SELECT
    ...
    booking_price,
    frequent_flyer_id
from dbt.dbt_oclarkecore.bookings,
LATERAL FLATTEN(input => TRY_PARSE_JSON(flight_ids)) flatten
) subquery
LEFT JOIN dbt.dbt_oclarkecore.flights ON subquery.flight_id = flights.flight_id
LEFT JOIN dbt.dbt_oclarkecore.customers ON subquery.frequent_flyer_id = customers.frequent_flyer_id
)
